...
@TeleOp(name = "F: Teleop Ultimate Red (FAR end); FAR AUTO")

public class TeleopUltimateRed extends NrpOpMode {
...
//todo
//todo
        //pedro turret stuff//todo
        follower = Constants.createFollower(hardwareMap);//todo
        //start with end of auto//todo
        follower.setStartingPose(new Pose(102.700, 20.000, Math.toRadians(25))); //TODO: Set for End of Auto
        follower.update();//todo
//todo
//todo
//todo
        try {
...
//                 In memento Ioanni Austinis Hatchionis
                ...
                if (gamepad1.x) {//corner [Square]
                    follower.setPose(new Pose(8.750, 7.00, Math.toRadians(180)));
                    follower.update();
                }

                if (gamepad1.y) { // auto init spot [Triangle]
//                    follower.setPose(new Pose(126.1, 120.80, Math.toRadians(40.7)));
                    follower.setPose(new Pose(125.5, 118.7, Math.toRadians(37)));
                    follower.update();
                }
                if (gamepad1.a) { // at gate [The X]
                    follower.setPose(new Pose(129.1, 70.5, Math.toRadians(0)));
                    follower.update();
                }
                if (gamepad1.b) { // exact auto start  [Circle]
                    follower.setPose(new Pose(125.500, 118.700, Math.toRadians(35)));
...
//
                ...
                double pos = 0.75 - (1/(2*Math.PI))*Math.atan((-1*turretY+132)/(-1*turretX+136)) + moddedRobotHeadingRad;
                if (pos > 1){
                    pos --;
                }
